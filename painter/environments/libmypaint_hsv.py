...
class LibMyPaint_hsv(environment.Environment):
  ...
  H_VALUES = np.linspace(0.0, 1.0, 20)
  S_VALUES = np.linspace(0.0, 1.0, 20)
  V_VALUES = np.linspace(0.0, 1.0, 20)
  ...
  def __init__(self,
    ...
    self._action_spec = collections.OrderedDict([
        ...
        ("hue", specs.DiscreteArray(len(self.H_VALUES))),
        ("saturation", specs.DiscreteArray(len(self.S_VALUES))),
        ("value", specs.DiscreteArray(len(self.V_VALUES)))])
  ...
  def _get_canvas(self):
    ...
    canvas = np.single(utils._fix15_to_hsva(buf))
    return canvas
  ...
  def _update_brush_params(self, **kwargs):
    ...
    if any(k in kwargs for k in hsv_keys):
      assert all(k in kwargs for k in hsv_keys)
      hue, saturation, value = [kwargs[k] for k in hsv_keys]
      for k in hsv_keys:
        del kwargs[k]
      if self._use_color:
        kwargs.update(dict(
  ...
  def _reset_brush_params(self):
    hue, saturation, value = self.H_VALUES[0], self.S_VALUES[0], self.V_VALUES[0]
  ...
  def _bezier_to(self, y_c, x_c, y_e, x_e, pressure,
                 log_size, hue, saturation, value):
    self._update_brush_params(
        y=y_e, x=x_e, pressure=pressure, log_size=log_size,
        hue=hue, saturation=saturation, value=value, is_painting=True)
  ...
  def _process_action(self, action):
    ...
    if self._use_size:
      ...
      log_size = self._log_brush_sizes[0]
    if self._use_color:
      hue = self.H_VALUES[action["hue"]]
      saturation = self.H_VALUES[action["saturation"]]
      value = self.V_VALUES[action["value"]]
    else:
    ...
    return locations, flag, pressure, log_size, hue, saturation, value
  ...
  def step(self, action):
    ...
    locations, flag, pressure, log_size, hue, saturation, value = (
    ...
    if flag == 1:  # The agent produces a visible stroke.
      ...
      y_c, x_c = loc_control
      y_e, x_e = loc_end
      self._bezier_to(y_c, x_c, y_e, x_e, pressure, log_size, hue, saturation, value)
